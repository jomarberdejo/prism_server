...
export const mailer = {
  ...
  async sendPasswordResetOTP(email: string, name: string, otp: string) {
    const html = `
      ...
      <html lang="en">
        <head>
          <meta charset="UTF-8" />
          <meta name="viewport" content="width=device-width, initial-scale=1.0"/>
          <title>Password Reset — cPrism</title>
        </head>
        <body style="margin:0;padding:0;background-color:#f8fafc;font-family:Arial,sans-serif;">
          <table width="100%" cellpadding="0" cellspacing="0" style="padding: 40px 0;">
            <tr>
              <td align="center">
                <table width="600" cellpadding="0" cellspacing="0" style="background:#ffffff;border-radius:12px;overflow:hidden;border:1px solid #e2e8f0;">
                  ...
                  <tr>
                    <td style="background-color:#ffffff;padding:36px 40px 24px;text-align:center;border-bottom:1px solid #e2e8f0;">
                      <img src="${LOGO_URL}" alt="LGU Carigara Seal" width="64" height="64"
                        style="display:block;margin:0 auto 16px;" />
                      <p style="margin:0;font-size:26px;font-weight:bold;color:#0f172a;letter-spacing:2px;">cPrism</p>
                      <p style="margin:5px 0 0;font-size:11px;color:#94a3b8;letter-spacing:0.5px;">LGU Carigara</p>
                    </td>
                  </tr>
                  ...
                  <tr>
                    <td style="padding:40px;background-color:#ffffff;">
                      ...
                      <p style="margin:0 0 24px;font-size:15px;color:#334155;line-height:1.8;">
                        We received a request to reset your cPrism password. Use the OTP code below. It expires in <strong>10 minutes</strong>.
                      </p>
                        ...
                        <tr>
                          <td align="center">
                            <div style="display:inline-block;background:#eff6ff;border:2px dashed #3b82f6;border-radius:12px;padding:20px 40px;">
                              <p style="margin:0;font-size:36px;font-weight:bold;color:#1d4ed8;letter-spacing:12px;">
                                ${otp}
                              </p>
                            </div>
                          </td>
    ...
    const response = await resend.emails.send({
      from: `cPrism <no-reply@${envConfig.RESEND_DOMAIN}>`,
      to: email,
      subject: "Password Reset OTP — cPrism",
      html,
    });
